...
pcb_s* create_process(func_t entry)
{
	...
	pcb->next_task = current_process->next_task;
	current_process->next_task = pcb;
...
{
	pcb_s *pcb = *((pcb_s**) args);
	reg_t *suivant = (*((pcb_s**) args))->context;
	reg_t* courant = current_process->context;
	reg_t* pile =  args;
	reg_t* current_lr = &(current_process->lr_user);	
	...
	__asm__("mov %[courantcpsr], r6" :
				[courantcpsr]"=r"(current_process->cpsr)
			);
