...
void bash_process()
{
	char buffer[BUFFER_CMD] = "echo test";
	while (1)
	{
		printf("\n");
		...
		else if (strcmp (cmdExploded[0], "lorem"))
			doLorem(0, NULL);
...
void doDrawRect(char argv, char **args)
{
	...
	unsigned int red = atoi(args[5])%255;
	unsigned int green = atoi(args[6])%255;
	unsigned int blue = atoi(args[7])%255;
...
void doDrawText(char argv, char **args)
{
	...
	unsigned int x = atoi(args[1]);
	unsigned int y = atoi(args[2]);
	unsigned int red = atoi(args[3])%255;
	unsigned int green = atoi(args[4])%255;
	unsigned int blue = atoi(args[5])%255;
